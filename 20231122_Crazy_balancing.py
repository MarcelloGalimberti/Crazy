...
st.subheader('Graficul de performanță | Grafico performance')
...
st.subheader('Alegeți numărul de soluții | Scegliere numero di soluzioni')
n = st.number_input('Numărul de soluții | Numero di soluzioni', step =1)

performance = performance[:n]
performance.reset_index(drop = True, inplace=True)

st.write('Performanțele soluțiilor sunt: | Le performance delle soluzioni sono:')
...
st.header ('Scenarii | Scenari', divider='violet')

for i in range (n):
    st.subheader (f'Scenariu | Scenario {i+1}')
    st.write('Rezumatul liniei | Summary linea')
    st.dataframe(lista_df_raw[i], width=1500)
    st.write ('Perechi | Abbinamenti')
    st.dataframe(lista_abbinamenti[i])
    st.write('Grafic cu linii | Grafo linea')
